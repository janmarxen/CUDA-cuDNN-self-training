...
__global__ void  NP_REG_k(float *x)
{
	/*************************************************************
...
// Local memory based solution
__global__ void  NP_LOC_k(float* x)
{
	int idx = threadIdx.x + blockIdx.x*blockDim.x;

    float param_0 = 0.2316419f;
    float param_1 = 0.3193815f;
    float param_2 = -0.3565638f;
    float param_3 = 1.781478f;
    float param_4 = -1.821256f;
    float param_5 = 1.330274f;
    float param_6 = 0.3989423f;
  ...
  if(X >= 0.0f){
      t = 1.0f / ( 1.0f + param_0 * X );
      x[idx] = (1.0f - param_6 * expf( -X * X / 2.0f ) * t *
			   ( t *( t * ( t * ( t * param_5 + param_4 ) +
			   param_3 ) + param_2 ) + param_1 ));
  }else{/* X < 0 */
      t = 1.0f / ( 1.0f - param_0 * X );
      x[idx] = ( param_6 * expf( -X * X / 2.0f ) * t *
			   ( t *( t * ( t * ( t * param_5 + param_4 ) +
			   param_3 ) + param_2 ) + param_1 ));
...
// Global memory based solution with fewer data stored
__global__ void  NP_GLOwSync_k(float* x)
{
    int idx = threadIdx.x + blockIdx.x*blockDim.x;
    __shared__ float sGlob[7];  // shared by all threads in the block

  // Thread 0 initializes shared memory
    if(threadIdx.x == 0){
        sGlob[0+blockIdx.x*NB] = 0.2316419f;
        sGlob[1+blockIdx.x*NB] = 0.3193815f;
        sGlob[2+blockIdx.x*NB] = -0.3565638f;
        sGlob[3+blockIdx.x*NB] = 1.781478f;
        sGlob[4+blockIdx.x*NB] = -1.821256f;
        sGlob[5+blockIdx.x*NB] = 1.330274f;
        sGlob[6+blockIdx.x*NB] = 0.3989423f;
    }
    __syncthreads();
  ...
  if(X >= 0.0f){
      t = 1.0f / ( 1.0f + sGlob[0] * X );
      x[idx] = (1.0f - sGlob[6] * expf( -X * X / 2.0f ) * t *
			   ( t *( t * ( t * ( t * sGlob[5] + sGlob[4] ) +
			   sGlob[3] ) + sGlob[2] ) + sGlob[1] ));
  }else{/* X < 0 */
      t = 1.0f / ( 1.0f - sGlob[0] * X );
      x[idx] = ( sGlob[6] * expf( -X * X / 2.0f ) * t *
			   ( t *( t * ( t * ( t * sGlob[5] + sGlob[4] ) +
			   sGlob[3] ) + sGlob[2] ) + sGlob[1] ));
...
void NP_GPU(float *a, float *b, int flag, float *TimerAdd){
	...
	if (flag == 1)	NP_GLOwSync_k<<<NB, NTPB>>>(aGPU);
	if (flag == 2)	NP_LOC_k<<<NB, NTPB>>>(aGPU);
	if (flag == 3)	NP_REG_k<<<NB, NTPB>>>(aGPU);
...
int main (void){
	...
	printf("Execution time using smaller global: %f ms\n", TimerAdd);
	printf(" %f is equal to %f \n", b[25], NP(a[25])); // Check the result
	NP_GPU(a, b, 2, &TimerAdd);
	printf("Execution time using local memory: %f ms\n", TimerAdd);
	printf(" %f is equal to %f \n", b[250], NP(a[250])); // Check the result 
	NP_GPU(a, b, 3, &TimerAdd);
	printf("Execution time using registers: %f ms\n", TimerAdd);
